...
def test_collect_metrics_for_models(collect_qc_service_balsamic_wgs: CollectQCService):

    # GIVEN a collect qc request
    expected_keys: list[str] = [
        collect_qc_service_balsamic_wgs.request.sample_ids[0],
        collect_qc_service_balsamic_wgs.request.sample_ids[1],
        collect_qc_service_balsamic_wgs.request.case_id,
    ]

    # WHEN collecting the metrics for the models
    collected_metrics: list[dict] = collect_qc_service_balsamic_wgs.collect_metrics()
...
def test_collect_balsamic_metrics_wgs(collect_qc_service_balsamic_wgs: CollectQCService):
    # GIVEN a collect qc request for a balsamic workflow

    # WHEN collecting the qc metrics
    balsamic_metrics: CollectQCResponse = (
        collect_qc_service_balsamic_wgs.collect_qc_metrics_for_request()
    )
...
def test_collect_balsamic_metrics_tga(collect_qc_service_balsamic_tga: CollectQCService):
    # GIVEN a collect qc request for a balsamic workflow

    # WHEN collecting the qc metrics
    balsamic_metrics: CollectQCResponse = (
        collect_qc_service_balsamic_tga.collect_qc_metrics_for_request()
    )
